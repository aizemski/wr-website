...
## Hero image gate (White Raven)

Before `BLOCKING: false`, verify:

- [ ] `heroImage` frontmatter is set and **not** a `blog-placeholder-*.jpg` path (unless explicitly dev-only)
- [ ] Referenced file exists under `src/assets/blog/` (or valid `src/assets/` path)
- [ ] Hero visual aligns with post title/persona (spot-check if asset is readable)

If hero step was skipped, set `BLOCKING: true` with reason: hero image missing—run `blog-image` agent.
