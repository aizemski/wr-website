...
## Project context (White Raven)

Before writing, load when present:

- `BRAND.md` and `VOICE.md` at the project root
- `docs/blog/personas.md` for buyer persona (Startup Steve, Enterprise Elena, Operations Oliver, Small Business Sarah)
- `docs/blog/topics-and-tags.md` for persona, service, technology, and industry hubs
- `src/utils/insights.ts` for article URLs (`/blog/{persona}/{slug}` or `/blog/services/{service}/{slug}`)
- `docs/blog/writing-guide.md` for structure, checklist, and **15–20 min read** length (~3,000–4,000 words)
- `docs/blog/hero-image.md` — hero is created **after** the draft (orchestrator runs `blog-image` agent)
...
8. Set frontmatter with taxonomy tags. Do **not** use `blog-placeholder-*.jpg` for publishable posts. After save, the orchestrator must run **`blog-image`** to create `src/assets/blog/{slug}.png` and set `heroImage` (see `docs/blog/hero-image.md`).
...
- [ ] Inline `[IMAGE: ...]` markers for body visuals (not the main hero—hero is post-write)
- [ ] Hero image: deferred to `blog-image` agent after draft is saved (or placeholder noted for dev)
- [ ] Every inline image URL was verified by the researcher (Verified column = Yes)
