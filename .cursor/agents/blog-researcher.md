...
3. Do NOT call MCP tools directly. The **`blog-image` Cursor agent** generates the main hero **after** the post is written (`docs/blog/hero-image.md`). Inline illustrations may use stock URLs from this research packet.
...
### Images Found ([N] total — inline only)

Main **hero/cover** is not sourced here. After `blog-writer` finishes, the orchestrator runs **`blog-image`** to generate `src/assets/blog/{slug}.webp` and update frontmatter.
...
|---|-----------|-------------|---------------------|
| 1 | hero (for blog-image agent) | [Editorial/Product/etc.] | [description derived from topic—refined after draft] |
```

## Cover / hero images (White Raven)

**Do not** search stock sites for the main hero. The **`blog-image` agent** runs after the article is saved and generates a local asset under `src/assets/blog/`.

Optional: include a **Hero concept** row in "AI Image Recommendations" to seed the post-write agent. Focus stock image research on **inline** `[IMAGE: ...]` placements only.

## Inline image search

When finding inline images:
